...
public class WebnovelServiceImpl implements WebnovelService {
	...
	private void uploadCover(ProductVO product, MultipartFile file) {
		try (InputStream is = file.getInputStream()) {
			...
			PutObjectRequest objectRequest = PutObjectRequest.builder()
					.bucket(bucketName)
					.key("webnovel/" + product.getId() + "/cover/" + file.getOriginalFilename()) // aws에 "webnovel/product id번호/cover/" 폴더에 파일 저장
	...
	private void uploadFile(ProductVO product, MultipartFile file) {
		try (InputStream is = file.getInputStream()) {
			...
			PutObjectRequest objectRequest = PutObjectRequest.builder()
					.bucket(bucketName)
					.key("webnovel/" + product.getId() + "/file/" + file.getOriginalFilename()) // aws에 "webnovel/product id번호/file/" 폴더에 파일 저장
					.contentType(file.getContentType())
					.acl(ObjectCannedACL.PUBLIC_READ)
					.build();
	...
	private void removeCover(ProductVO product) {
		String key = "webnovel/" + product.getId() + "/cover/" + product.getFile_name();
	...
	private void removeFile(ProductVO product) {
		String key = "webnovel/" + product.getId() + "/file/" + product.getFile_name();
